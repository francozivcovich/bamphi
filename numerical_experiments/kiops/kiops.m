...
% evaluated at tA acting on vectors from u.
...
%
% Task I (which is the default with task1 == true) evaluates
%
% w(:,i) = phi_k(T(i) A) u(:, k)
%
% for i = 1:length (T). It is important that u(:,1:k-1) is zero.
%
% Task II evaluates
%
% w(:,i) = phi_0(T(i) A) u(:, 1) + T(i) * phi_1(T(i) A) u(:, 2) + ...
%          T(i)^2 * phi_2(T(i) A) u(:, 3) + ...
%
% for i = 1:length (T).
%
...
V = zeros(      n + p, m_init + 1 );
H = zeros( m_init + 1, m_init + 1 );
